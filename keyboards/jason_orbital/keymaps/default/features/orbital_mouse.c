 ...
 */
 ...
 // Instantly move the Orbital Mouse by a small increment in the forward ("up") direction.
 // Call this function externally to move the mouse by a single step instantly.
 ...
 void orbital_mouse_instant_step(int dir) {
   ...
   if (dir == 0) return; // No movement if direction is zero.
